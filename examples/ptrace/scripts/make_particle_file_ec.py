...
# 3962686.6867961055 572559.8476294627 -4955991.1635930547

def southern_ocean_only_xyz(x, y, z, maxNorth=-45.0): #{{{
    sq = np.sqrt((x - 3962686.6867961055)**2 + (y - 572559.8476294627)**2 +  (z + 4955991.1635930547)**2)
    ids = sq < 1#1823554.80847
    return ids #}}}
...
class ParticleList(): #{{{
    ...
    def write(self, f_name, f_decomp):  #{{{
       ...
       #         'Number of particles must be larger than decomposition!'
        ...
        f_out.variables['currentCell'][0,:] = self.cellindices
